...
def define_model(args):
    ...
    if args.task == 'classical_sr':
        model = net(upscale=args.scale, in_chans=1, img_size=args.training_patch_size, window_size=8,
...
def get_image_pair(args, path):
    ...
    if args.task in ['classical_sr', 'lightweight_sr']:
        img_gt = cv2.imread(path, cv2.IMREAD_GRAYSCALE)
        img_lq = cv2.imread(f'{args.folder_lq}/{imgname}x{args.scale}{imgext}', cv2.IMREAD_GRAYSCALE )
